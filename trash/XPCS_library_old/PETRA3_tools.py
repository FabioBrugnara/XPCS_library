...
# BEAM CENTER AND DETECTOR DIMENSION
...
def load_fio(raw_folder, file_name, Nscan):
    '''
    ...
    fio_data['command'] = fio_lines[find_line('%c\n', fio_lines)+1].strip()
    ...
    try:
        ...
        fio_data['data'] = pd.read_csv(fio_file, skiprows=first_dline, skipfooter=1, names=cols, sep=' ', engine='python')
        fio_data['data'].reset_index(drop=True, inplace=True)
    except: pass
...
def get_att(fio_data):
    '''
    Get the attenuation ijnteger as expressed in the shell from the fio file data.

    Args:
        fio_data (dict): the fio data
    
    Returns:
        att (int): the attenuation
    '''
    
    abs1z = round(fio_data['params']['abs1z'])
    abs2z = round(fio_data['params']['abs2z'])

    att_dict = {(0,0):0, (1,0):1, (0,1):2, (1,1):3, (2,0):4, (0,2):5, (2,1):6, (1,2):7, (2,2):8}

    return att_dict[(abs1z, abs2z)]
...
def read_e4m_itime(raw_folder, file_name, Nscan):
    '''
    Read the integration time from the e4m master file (only for e4m scans).
    ...
    Returns:
        e4m_itime (float): the integration time of the e4m scan
    '''
...
def read_e4m_wl(raw_folder, file_name, Nscan):
    '''
    Read the wavelength from the e4m master file (only for e4m scans).
    ...
    Returns:
        e4m_wl (float): the wavelength of the e4m scan
    '''
...
def save_sparse_e4m(OF, sA, raw_folder, file_name, Nscan):
    ...
    sparse.save_npz(e4m_sparse_file, sA)
...
def load_sparse_e4m(raw_folder, file_name, Nscan):
    ...
    e4m_sparse_file   = raw_folder + file_name+'_'+str(Nscan).zfill(len_scan_string) + '/e4m/' + file_name+'_'+str(Nscan).zfill(len_scan_string)+'_sparse.npz'
    e4m_overflow_file = raw_folder + file_name+'_'+str(Nscan).zfill(len_scan_string) + '/e4m/' + file_name+'_'+str(Nscan).zfill(len_scan_string)+'_overflow.npy'
    sA = sparse.load_npz(e4m_sparse_file)
    if sparse.isspmatrix_coo(sA): sA = sparse.coo_array(sA)
    elif sparse.isspmatrix_csc(sA): sA = sparse.coo_array(sA.tocoo())
    elif sparse.isspmatrix_csr(sA): sA = sparse.coo_array(sA.tocoo())
